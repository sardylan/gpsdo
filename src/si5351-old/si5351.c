...
enum PLL pll_assignment[8];
uint64_t clk_freq[8];
uint64_t plla_freq;
uint64_t pllb_freq;
enum PLLInput plla_ref_osc;
enum PLLInput pllb_ref_osc;
uint32_t xtal_freq[2];

// private:
uint64_t pll_calc(enum PLL, uint64_t, struct Si5351RegSet *, int32_t, uint8_t);
...
void ms_div(enum Clock, uint8_t, uint8_t);
...
void si5351_reset(void) {
    ...
    for (i = 0; i < 8; i++) {
        clk_freq[i] = 0;
        si5351_output_enable((enum Clock) i, 0);
 ...
 */
uint8_t si5351_set_freq(uint64_t freq, enum Clock clk) {
            ...
            for (i = 0; i < 6; i++) {
                if (clk_freq[i] != 0) {
                    if (pll_assignment[i] == pll_assignment[clk]) {
                        ...
                        set_ms((enum Clock) i, temp_reg, int_mode, r_div, div_by_4);
                    }
                }
            }
 ...
 *   (use the si5351_clock enum)
 */
uint8_t set_freq_manual(uint64_t freq, uint64_t pll_freq, enum Clock clk) {
    struct Si5351RegSet ms_reg;
    uint8_t int_mode = 0;
    uint8_t div_by_4 = 0;
 ...
 */
void set_pll(uint64_t pll_freq, enum PLL target_pll) {
 ...
 *   Set to 1 to enable, 0 to disable
 */
void set_ms(enum Clock clk, struct Si5351RegSet ms_reg, uint8_t int_mode, uint8_t r_div, uint8_t div_by_4) {
    uint8_t __p[20];
    uint8_t *params = __p;
 ...
 */
void si5351_output_enable(enum Clock clk, uint8_t enable) {
    uint8_t reg_val;
 ...
 */
void si5351_drive_strength(enum Clock clk, enum Drive drive) {
    uint8_t reg_val;
 ...
 * crystal.
 */
void set_correction(int32_t corr, enum PLLInput ref_osc) {
 ...
 */
void set_phase(enum Clock clk, uint8_t phase) {
 ...
 * in RAM.
 */
int32_t get_correction(enum PLLInput ref_osc) {
 ...
 */
void pll_reset(enum PLL target_pll) {
    if (target_pll == SI5351_PLLA) {
 ...
 */
void set_ms_source(enum Clock clk, enum PLL pll) {
    uint8_t reg_val;
 ...
 */
void set_int(enum Clock clk, uint8_t enable) {
    uint8_t reg_val;
    reg_val = si5351_read(SI5351_CLK0_CTRL + (uint8_t) clk);
 ...
 * saving feature).
 */
void si5351_set_clock_pwr(enum Clock clk, uint8_t pwr) {
    uint8_t reg_val; //, reg;
    reg_val = si5351_read(SI5351_CLK0_CTRL + (uint8_t) clk);
 ...
 */
void set_clock_invert(enum Clock clk, uint8_t inv) {
    uint8_t reg_val;
    reg_val = si5351_read(SI5351_CLK0_CTRL + (uint8_t) clk);
 ...
 * the clock output.
 */
void set_clock_source(enum Clock clk, enum ClockSource src) {
    uint8_t reg_val;
    reg_val = si5351_read(SI5351_CLK0_CTRL + (uint8_t) clk);
 ...
 */
void set_clock_disable(enum Clock clk, enum ClockDisable dis_state) {
    uint8_t reg_val, reg;
 ...
 */
void set_clock_fanout(enum ClockFanout fanout, uint8_t enable) {
    uint8_t reg_val;
 ...
 */
void set_pll_input(enum PLL pll, enum PLLInput input) {
    uint8_t reg_val;
 ...
 */
void set_ref_freq(uint32_t ref_freq, enum PLLInput ref_osc) {
    // uint8_t reg_val;
    //reg_val = si5351_read(SI5351_PLL_INPUT_SOURCE);
...
uint64_t pll_calc(enum PLL pll, uint64_t freq, struct Si5351RegSet *reg, int32_t correction, uint8_t vcxo) {
    uint64_t ref_freq;
    if (pll == SI5351_PLLA) {
...
void ms_div(enum Clock clk, uint8_t r_div, uint8_t div_by_4) {
    uint8_t reg_val = 0;
    uint8_t reg_addr = 0;
...
uint8_t si5351_write_bulk(uint8_t regAddr, uint8_t length, uint8_t *data) {
    ...
    i2c_write_blocking(ctx.bus, i2c_bus_addr, msg, (length + 1), false);
...
uint8_t si5351_read(uint8_t regAddr) {
    ...
    i2c_write_blocking(ctx.bus, i2c_bus_addr, &regAddr, 1, true);
    i2c_read_blocking(ctx.bus, i2c_bus_addr, &buf, 1, false);
